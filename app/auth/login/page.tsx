...
function LoginContent() {
    ...
    const handleGoogleLogin = async () => {
        if (Capacitor.isNativePlatform()) {
            ...
            // abría el activity de Google y en gama media (Redmi/Samsung) el proceso
            // del WebView moría al volver -> la app se cerraba al tocar Google.
            setLoading(true);
            setLoginError(null);
            setStep('Abriendo Google...');
            try {
                await signIn('google', { callbackUrl: '/dashboard', redirect: true });
            } catch (e: any) {
                setLoading(false);
                setStep('');
                setLoginError(e?.message || 'Error al iniciar con Google.');
            }
        } else {
            signIn('google', {
    ...
    return (
        <div className="min-h-screen w-full flex items-center justify-center p-6 relative overflow-hidden">
            ...
            <div className="absolute top-[-20%] left-[-20%] w-[60vw] h-[60vw] bg-primary/20 rounded-full blur-[50px] md:blur-[120px] pointer-events-none" />
            <div className="absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] bg-accent/10 rounded-full blur-[40px] md:blur-[100px] pointer-events-none" />
            ...
            >
                ...
                {/* Login Card */}
                <div className="glass-panel p-6 md:p-8 space-y-5 md:space-y-6 max-[340px]:p-5">
                    <form onSubmit={handleLogin} className="space-y-5">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-3 md:py-3.5 btn-primary text-xs md:text-sm font-bold flex items-center justify-center gap-2"
                        >
                            {loading ? (
                    ...
                    <div className="space-y-3">
                        ...
                        >
                            {loading ? (
                                <>
                                    <Loader2 className="w-5 h-5 animate-spin" />
                                    <span className="tracking-[0.1em]">{step || 'Google'}</span>
                                </>
                            ) : (
                                <>
                        ...
                        {loginError && (
                            <p className="text-center text-[11px] text-red-400 font-medium leading-snug">
                                {loginError}
                            </p>
                        )}
                    </div>
