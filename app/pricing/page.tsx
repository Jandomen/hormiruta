...
export default function PricingPage() {
    ...
    const handleApprove = async (orderId: string, planName: string) => {
        ...
        try {
            ...
            if (response.ok) {
                alert(`¡Pago exitoso! Bienvenido al plan ${planName}`);
                window.location.href = '/dashboard';
            } else {
                alert('Hubo un problema activando tu suscripción. Contacta a soporte.');
            }
        } catch (error) {
            console.error(error);
                            ...
                            >
                                ...
                                {plan.price === '$0' ? (
                                    ...
                                    <div className="w-full min-h-[50px]">
                                        <PayPalButtons
                                            style={{ layout: "horizontal", tagline: false, height: 45, color: 'blue', label: 'pay' }}
                                            createOrder={(data, actions) => {
                                                const value = plan.price.replace('$', '');
                                        ...
                                        <button
                                            ...
                                            className="w-full mt-2 text-[10px] text-white/40 uppercase hover:text-white"
                                        >
                                            Cancelar
                                        </button>
                                    </div>
