            ...
            >
                <div className="p-3 sm:p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                    <div className="min-w-0">
                        ...
                        <p className="text-[7px] sm:text-[9px] font-bold text-white/30 uppercase tracking-widest mt-0.5">Historial y Planificación</p>
                    </div>
                    <button onClick={onClose} className="p-2 sm:p-3 hover:bg-white/5 rounded-xl sm:rounded-2xl transition-colors text-white/40 hover:text-white shrink-0">
                ...
                <div className="p-3 sm:p-6 max-h-[50vh] overflow-y-auto custom-scrollbar">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-8 sm:py-16 space-y-2 sm:space-y-3">
                            ...
                            <p className="text-[8px] sm:text-[9px] font-black text-white/20 uppercase tracking-[0.2em]">Accediendo...</p>
                        </div>
                    ) : error ? (
                        <div className="p-3 sm:p-4 bg-red-500/10 border border-red-500/20 rounded-lg sm:rounded-xl text-red-500 text-[9px] sm:text-[10px] italic text-center">
                            {error}
                        </div>
                    ) : routes.length === 0 ? (
                        <div className="text-center py-8 sm:py-16 space-y-3 sm:space-y-4 opacity-30">
                            ...
                            <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest">No hay rutas guardadas</p>
                        </div>
                    ) : (
                                ...
                                >
                                    <div className="flex items-center gap-2 sm:gap-4 overflow-hidden">
                                        ...
                                        <div className="overflow-hidden">
                                            <h3 className="text-white font-black text-[9px] sm:text-xs uppercase tracking-tight truncate italic">{route.name}</h3>
                                            <div className="flex items-center gap-1 sm:gap-2 mt-0.5 truncate">
                                                <span className="text-[7px] sm:text-[9px] font-black text-white/30 uppercase tracking-widest">{formatDate(route.date)}</span>
                                                <span className="w-0.5 h-0.5 rounded-full bg-white/10 shrink-0" />
                                                <span className="text-[7px] sm:text-[9px] font-black text-info uppercase tracking-widest">{route.stops.length} Paradas</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                                        ...
                                        <ChevronRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white/10 group-hover:text-white/40 transition-colors shrink-0" />
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="p-3 sm:p-6 bg-white/[0.02] border-t border-white/5">
                    <button
                        onClick={onClose}
                        className="w-full py-3 sm:py-4 rounded-lg sm:rounded-2xl border border-white/10 text-white font-black uppercase text-[8px] sm:text-[10px] tracking-widest hover:bg-white/5 transition-all"
                    >
                        Cerrar
                    </button>
                </div>
