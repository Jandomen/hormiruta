        ...
        <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-3 sm:p-8 rounded-2xl sm:rounded-[32px] space-y-3 sm:space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-white/50 text-[10px] sm:text-xs uppercase tracking-[0.2em] font-bold truncate">Protocolo Directo Activo</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col items-start sm:items-end gap-1 shrink-0">
                    {lastSync && (
                        <div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20">
                            ...
                            <span className="text-[10px] sm:text-xs font-black text-blue-300 uppercase tracking-tighter">Sync: {lastSync}</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-3 sm:space-y-6">
                <div className="space-y-1.5 sm:space-y-2">
                    <label className="text-[10px] sm:text-sm font-black text-white/60 uppercase tracking-[0.2em] pl-1">Contacto de Emergencia</label>
                    <div className="relative group">
                        <input
                            ...
                            className="w-full bg-black/60 border border-white/10 rounded-lg sm:rounded-2xl px-3 sm:px-8 py-2 sm:py-6 text-white text-xs sm:text-lg focus:outline-none focus:border-red-500/50 transition-all placeholder:text-white/40 shadow-inner"
                            required
                        />
                        <div className="absolute top-1/2 -translate-y-1/2 right-3 sm:right-6 text-white/40 group-focus-within:text-red-500/50 transition-colors">
                ...
                <div className="space-y-3 sm:space-y-4">
                    <button
                        ...
                        className={cn(
                            "w-full font-black py-3 sm:py-6 rounded-lg sm:rounded-2xl flex items-center justify-center gap-2 sm:gap-4 transition-all shadow-2xl active:scale-[0.98] text-[10px] sm:text-sm uppercase tracking-[0.2em]",
                    ...
                    <AnimatePresence mode="wait">
                        {message && (
                            <motion.div
                                ...
                                className={cn(
                                    "p-3 sm:p-4 rounded-xl sm:rounded-2xl flex items-center gap-2 sm:gap-3 text-[10px] sm:text-xs font-black uppercase tracking-widest border shadow-xl",
            ...
            <div className="flex gap-2 sm:gap-4 p-2 sm:p-4 bg-white/5 rounded-lg sm:rounded-2xl border border-white/5 items-start">
                <AlertCircle className="w-3 h-3 sm:w-4 sm:h-4 text-white/50 mt-0.5 shrink-0" />
                <p className="text-[10px] sm:text-xs text-white/70 italic leading-relaxed font-medium break-words">
